...
def apply_filter(freq, bandwidth, sample_rate):
    if freq == 0:  # this is a highpass
        ...
        b, a = low_pass(cutoff_freq=freq - bandwidth /
...
def auto_filter(self, bandwidth=4, plot_peaks=False):
    ...
    f, Pxx_den = signal.welch(
        self.data['Ex'], fs=self.sample_rate, nperseg=self.sample_rate)
    ...
    f, Pxx_den = signal.welch(
        self.data['Ex'], fs=self.sample_rate, nperseg=nperseg)
...
def despike(data, window_size=50, fp_kwargs={}, flip=False):

    if flip:
        peaks, properties = signal.find_peaks(-data, **fp_kwargs)
    else:
        peaks, properties = signal.find_peaks(data, **fp_kwargs)

    # allows for assymetric windows
    if isinstance(window_size, list):
        n_1, n_2 = window_size
    else:
        n_1 = n_2 = int(window_size/2)

    # create a numpy array of all windows
    windows = np.array([data.iloc[i-n_1:i+n_2].to_numpy() for i in peaks])
  
    # print(windows[:100])
    # determine the mean value across all windows
    window_average = np.mean(windows,axis=0)

    # subtract the average window from each window
    # subtracted_windows = windows - window_average

    subtracted_windows = windows * (-signal.get_window('hamming',len(window_average))+1)


    # create a copy of the data and apply the filter to it
    despiked = data.copy()
    for new_values, peak in zip(subtracted_windows, peaks):    
        despiked.iloc[peak-n_1:peak+n_2] = new_values
    ...
    return despiked, windows, window_average, peaks
